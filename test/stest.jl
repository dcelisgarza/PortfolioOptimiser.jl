...
asset_statistics!(
    ...
    cor_settings = CorSettings(;
        method = :Gerber2,
        estimation = CorEstSettings(; estimator = AnalyticalNonlinearShrinkage()),
    ),
)
...
cov2cor(cov(method, portfolio.returns))

# cluster_assets!(portfolio, linkage = :ward)
asset_classes = DataFrame(
    "Assets" => portfolio.assets,
    "PDBHT" => [1, 2, 1, 1, 1, 3, 2, 2, 3, 3, 3, 4, 4, 3, 3, 4, 2, 2, 3, 1],
    "SPDBHT" => [1, 1, 1, 1, 1, 2, 3, 4, 2, 3, 3, 2, 3, 3, 3, 3, 1, 4, 2, 1],
    "Pward" => [1, 1, 1, 1, 1, 2, 3, 2, 2, 2, 2, 4, 4, 2, 3, 4, 1, 2, 2, 1],
    "SPward" => [1, 1, 1, 1, 1, 2, 2, 3, 2, 2, 2, 4, 3, 2, 2, 3, 1, 2, 2, 1],
    "G2DBHT" => [1, 2, 1, 1, 1, 3, 2, 3, 4, 3, 4, 3, 3, 4, 4, 3, 2, 3, 4, 1],
    "G2ward" => [1, 1, 1, 1, 1, 2, 3, 4, 2, 2, 4, 2, 3, 3, 3, 2, 1, 4, 2, 2],
)
constraints = DataFrame(
    ...
    Symbol("Type Relative") => ["", "", "Assets", "Assets", "", "", "Assets", "Assets"],
    :Relative => ["", "", "T", "T", "", "", "GM", "PFE"],
    Symbol("Relative Set") => ["", "", "", "", "", "", "", ""],
    :Factor => ["", "", 0.5, 2, "", "", 0.3, 2],
)
