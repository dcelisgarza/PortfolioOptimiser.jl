...
wt = [0.023212410493909395, 0.04180507918898045, 0.02097173779429894, 0.01915222066801841,
      0.019617512785329, 0.027315103218872105, 0.010288389735636377, 0.28698504210338543,
      0.018692638550135976, 0.024753790610823476, 0.04069430485974347, 0.08801583260757373,
      0.05317874944885358, 0.032218256478400265, 0.07437971688459784, 0.10407153529171204,
      0.028007972576333588, 0.03470276984884458, 0.022846121725202524, 0.029090815129348833]
@test isapprox(w1.weights, wt)

w2 = optimise!(portfolio, NCO(; internal = NCOArgs(; type = HERC(; rm = sd))))
wt = [0.03502677104953508, 0.034898398023022054, 0.03164566895904809, 0.02535795569178541,
      0.02789344989235047, 0.028583381104026976, 0.009422007575348099, 0.1299161111096586,
      0.023016264028061697, 0.024521990540045523, 0.03726744986476854, 0.0968547793732363,
      0.10704219566485063, 0.03191655746795579, 0.14971710111225556, 0.07389688676967159,
      0.03982354889701095, 0.034627489210387746, 0.02813045190138087, 0.03044154176560006]
               ...
               NCO(;
                   internal = NCOArgs(;
                                      type = NCO(;
                                                 internal = NCOArgs(;
                                                                    type = Trad(; rm = sd))))))
wt = [0.014970185332617464, 0.019969406223918334, 0.006075038357739269,
      0.0053985166058852325, 0.0013331753123613638, 0.0296248095357785,
      2.640106315365002e-8, 0.10411915977052497, 2.6334334779465288e-8, 0.01627264985341318,
      0.2707819347956372, 0.013353754143991238, 6.795230721847408e-8, 0.11853203646007114,
      1.5158879161878883e-7, 0.02460035080579429, 0.034110397825119015, 0.19553464910294138,
      3.75648576519955e-8, 0.14532362603285304]
@test isapprox(w3.weights, wt)

w4 = optimise!(portfolio; rm = sd, type = NCO(; opt_kwargs = (; type = Trad())))
wt = [0.007695637085032112, 0.03659084698695424, 0.006133443008929141, 0.025442455183730738,
      0.007681695657943142, 0.027291262001934883, 8.754947434845838e-9, 0.10682623597381134,
      1.421303588373142e-8, 0.007422207759121406, 0.2903401646808362, 0.013680462386839959,
      1.8064751257685872e-8, 0.1141795813527027, 4.029905570744343e-8, 0.022994697351611623,
      0.005724264413172602, 0.19842766800447242, 1.980131237893785e-8, 0.12956927701980497]
@test isapprox(w4.weights, wt)

w5 = optimise!(portfolio; rm = sd, type = NCO(; opt_kwargs = (; type = RP())))
wt = [0.02951361849950664, 0.031104849715876133, 0.02793314036244879, 0.02656314996044489,
      0.02698369883073134, 0.03534642287440858, 0.01670180592241015, 0.16962120247500095,
      0.024968721727516394, 0.030553868252817862, 0.053419634990817616, 0.08054389490707248,
      0.07156673974826498, 0.03978806514517936, 0.10009831144259478, 0.09468921373208417,
      0.030878020651025625, 0.043903261583117, 0.028415495601899526, 0.037406883576782755]
@test isapprox(w5.weights, wt, rtol = 0.0001)

w6 = optimise!(portfolio; rm = sd, type = NCO(; opt_kwargs = (; type = RRP())))
wt = [0.02951389405485954, 0.031104315451173856, 0.02793208917163586, 0.026562505049048737,
      0.026983103403796727, 0.035348278753467896, 0.016701824123189688, 0.1696245757506648,
      0.02496884681274182, 0.030553463074720424, 0.05342434151726439, 0.0805406657231586,
      0.07156201737329698, 0.039791579102299475, 0.10009185672120541, 0.09468677654596781,
      0.03087798304913965, 0.04390808127633267, 0.028416481902808967, 0.03740732114322675]
@test isapprox(w6.weights, wt)

w7 = optimise!(portfolio; rm = sd,
               type = NCO(;
                          wc_kwargs = (;
                                       wc_type = WCType(;
                                                        box = NormalWC(; seed = 123456789))),
                          opt_kwargs = (; type = WC())))
wt = [0.009564348753808937, 0.03203152793686595, 0.001688911518696649, 0.014014244982768006,
      5.588934815349284e-5, 0.024504649120183512, 7.512500556794461e-7, 0.1010164472946808,
      2.443365598914432e-6, 0.0003269297614983442, 0.30708159223955117,
      0.010608704805233413, 3.432728885546797e-6, 0.11778932427833054, 7.992047409051827e-6,
      0.019630375733017118, 0.01538142909936743, 0.2095035463243885, 5.1058392715841835e-6,
      0.13678235357223553]
@test isapprox(w7.weights, wt)

w8 = optimise!(portfolio; rm = sd, type = NCO(; opt_kwargs = (; type = NOC())))
wt = [0.046571868460761855, 0.05063463283102663, 0.04522234965984115, 0.042184874980313466,
      0.04843786710604607, 0.048149861232103144, 0.02614516308274899, 0.13968419663046644,
      0.038810092976760316, 0.04744714636404889, 0.0944384454107991, 0.020608863650339572,
      0.0005637870530288163, 0.060407285911415864, 0.0014574390260837916,
      0.05293577466466181, 0.05223809459315092, 0.07475074326336022, 0.0460649218256971,
      0.06324659127734568]
...
@testset "NCO WC" begin
    ...
    try
        ...
        @test isapprox(w2.weights, wt, rtol = 5.0e-8)
    catch err
    end
end
