using Test, PortfolioOptimiser, DataFrames, TimeSeries, CSV, Dates, ECOS, SCS,
      Clarabel, COSMO, OrderedCollections, LinearAlgebra, StatsBase, HiGHS,
      Logging
...
@testset "Efficient Frontier" begin
    ...
    frontier = efficient_frontier!(portfolio; kelly = :Approx, rm = :CVaR,
                                   points = 3)
    @test isapprox(frontier[:CVaR][:weights][!, "1"], limits.w_min)
    @test isapprox(frontier[:CVaR][:weights][!, "3"], limits.w_max)
    r1 = calc_risk(limits.w_min, portfolio.returns; rm = :CVaR,
                   alpha = portfolio.alpha)
    r2 = calc_risk(limits.w_max, portfolio.returns; rm = :CVaR,
                   alpha = portfolio.alpha)
...
@testset "SD" begin
    ...
    w3t = [7.179506307916457e-12, 1.914107597851177e-11, 2.442828242865303e-11,
           ...
           7.937026546069197e-12, 1.3137860840363715e-11,
           1.7427427416358682e-12, 1.2271864288937697e-12, 4.0970513336755e-12,
           1.2119635713300584e-12, 0.14326820230127735, 0.19649614109499838,
           1.5441931656159965e-11, 0.07852577342436559, 1.5813088271938218e-11]
    ...
    w4t = [1.492384419146211e-11, 1.6739191162167344e-11,
           2.0836013764845944e-11, 1.8751749610844453e-11, 0.8503252807188676,
           5.7961013000671885e-12, 0.14967471902985832, 1.0927909565621052e-11,
           1.7808151191717344e-11, 1.2061312430023853e-11,
           1.0494612386411761e-11, 5.9927999064637444e-12,
    ...
    w3t = [1.2366247299308615e-8, 3.8760594982905365e-8, 5.01769357793134e-8,
           ...
           0.140327354862629, 0.2133795775140911, 3.455573050831223e-8,
           0.0985355268380842, 3.268882511261784e-8]
    w4t = [2.697552976386826e-12, 3.020953795609185e-12, 3.753620425993217e-12,
           ...
           2.2255474150587952e-12, 1.9770094329998858e-12,
           1.2319437947926673e-12, 8.781818466077055e-13,
           1.5791159073970072e-12, 8.925281327890656e-13, 6.117590249323203e-12,
           4.204128068671476e-12, 2.165843521220285e-12, 3.502429128038115e-12,
    ...
    obj = :Sharpe
    portfolio.sd_u = r1
    w13 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf,
                    l = l)
    ...
    portfolio.mu_l = m2
    w10 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf,
                    l = l)
    ...
    portfolio.mu_l = m3
    w11 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf,
                    l = l)
    ...
    portfolio.mu_l = m4
    w12 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf,
                    l = l)
...
@testset "MAD" begin
    ...
    w3t = [3.6505434757561137e-11, 1.1760119951123315e-10,
           1.2484360660470936e-10, 6.187461102716296e-11, 0.6622657503882446,
           3.163239689478531e-11, 0.04259470199789395, 5.236792200043181e-11,
           1.294473002321869e-10, 4.625165845002466e-11, 9.65101902925865e-11,
           3.0215304348004426e-11, 4.614294245962137e-11, 4.394867706953358e-12,
           4.9837601080195894e-11, 0.13436659129606182, 0.08736657849319988,
           8.867633096743114e-11, 0.07340637681124025, 9.705810174060336e-11]
    w4t = [9.173670341082928e-11, 1.018721492422151e-10, 1.4792929705934207e-10,
           ...
           2.2582689785453977e-11, 4.587953990398343e-11,
           2.4076677157718124e-11, 2.388673459501035e-10, 1.627041098591065e-10,
           6.611088689517032e-11, 1.171673646256256e-10, 8.728903651047666e-11]
    ...
    w4t = [2.7462995127650345e-12, 3.4547354561297505e-12,
           4.638228249512704e-12, 4.027450273303031e-12, 0.8533951447671612,
           1.2491743001999807e-12, 0.14660485517629157, 2.2606929911136244e-12,
           4.074536117285144e-12, 2.5896147160714515e-12,
           2.1791822915117813e-12, 1.3835188712694755e-12,
           1.1710980464498196e-12, 1.6965332185628418e-12,
           1.1455634621293855e-12, 8.511463740765157e-12, 5.491779287346602e-12,
           2.368415950460791e-12, 4.427973683207736e-12, 3.130826651748219e-12]
...
@testset "SSD" begin
    ...
    w4t = [1.530418161094495e-10, 2.861231416908675e-10, 5.50565719114187e-10,
           ...
           3.8768506683038414e-10, 2.347497794488437e-10,
           4.1508405016915417e-10, 6.184903567113634e-10, 7.002855763348704e-10,
           3.106332338153356e-11, 4.812278883700028e-10, 1.6203871132884816e-10]
...
@testset "FLPM" begin
    ...
    w3t = [1.1587206453251757e-11, 3.9709992104292095e-11,
           4.209755573399237e-11, 2.317979270933299e-11, 0.6999124722828386,
...
@testset "Kurt full" begin
    ...
    w2t1 = [2.9227124385904432e-5,
            0.0402457135635424,
            0.054894083895073234,
            0.03947184860487557,
            0.3323558730566105,
            5.239724808999501e-5,
            0.07607741523015846,
            0.0227642769662796,
            0.039707677766003105,
            0.000142923857642769,
            0.0008269574422699847,
            2.2646351438318596e-6,
            5.151735747609655e-5,
            3.727605003653932e-5,
            9.195705877586154e-6,
            0.1324204831265113,
            0.13038373055233182,
            0.013643161833277745,
            0.08670707833599083,
            0.030176897648422765]
    w4t = [1.7725244472635677e-6,
...
@testset "OWA optimisations" begin
    ...
    w2 = opt_port!(portfolio; type = :Trad, rm = :OWA, obj = obj, rf = rf,
                   l = l)
    ...
    w4 = opt_port!(portfolio; type = :Trad, rm = :OWA, obj = obj, rf = rf,
                   l = l)
    ...
    obj = :Max_Ret
    portfolio.owa_u = r4
    w5 = opt_port!(portfolio; type = :Trad, rm = :OWA, obj = obj, rf = rf,
                   l = l)
    @test isapprox(w5.weights, w4.weights, rtol = 7e-8)

    obj = :Sharpe
    portfolio.owa_u = r2
    w6 = opt_port!(portfolio; type = :Trad, rm = :OWA, obj = obj, rf = rf,
                   l = l)
    ...
    portfolio.gmd_u = Inf
    portfolio.owa_u = r2
    w8 = opt_port!(portfolio; type = :Trad, rm = :OWA, obj = obj, rf = rf,
                   l = l)
    r8 = calc_risk(portfolio; rm = rm)
...
@testset "Constraints" begin
    ...
    obj = :Min_Risk
    w2 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test all(abs.(w2.weights - portfolio.turnover_weights) .<=
              portfolio.turnover)
    ...
    obj = :Sharpe
    w4 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test all(abs.(w4.weights - portfolio.turnover_weights) .<=
              portfolio.turnover)
    ...
    w2t = [0.00012937963825922666,
           ...
           0.00547678844034211]
    @test isapprox(w2t, w2.weights, rtol = 3e-4)
    @test norm(Matrix(returns[!, 2:end]) *
               (w2.weights - portfolio.tracking_err_weights),
    ...
    @test norm(Matrix(returns[!, 2:end]) *
               (w4.weights - portfolio.tracking_err_weights),
               2) / sqrt(nrow(returns) - 1) <= portfolio.tracking_err
    ...
    obj = :Sharpe
    w5 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test norm(Matrix(returns[!, 2:end]) * w5.weights -
               portfolio.tracking_err_returns, 2) /
    ...
    obj = :Min_Risk
    w6 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test norm(Matrix(returns[!, 2:end]) * w6.weights -
               portfolio.tracking_err_returns, 2) /
          sqrt(nrow(returns) - 1) <= portfolio.tracking_err
    ...
    w3 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test sum(w3.weights[abs.(w3.weights) .> 1e-6]) <=
          portfolio.max_number_assets

    obj = :Sharpe
    portfolio.max_number_assets = 11
    w4 = opt_port!(portfolio; rm = rm, obj = obj, kelly = kelly, rf = rf, l = l)
    @test sum(w4.weights[abs.(w4.weights) .> 1e-6]) <=
          portfolio.max_number_assets
end
