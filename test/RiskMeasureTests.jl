...
@testset "Risk measures" begin
    ...
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :Variance)[1] -
        0.000101665490230637,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :SD)[1] -
        0.010082930637004155,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :MAD)[1] -
        0.007418863748729646,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :SSD)[1] -
        0.007345533015355076,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :FLPM)[1] -
        0.0034827678064358134,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :SLPM)[1] -
        0.007114744825145661,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :WR)[1] -
        0.043602428699089285,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :VaR)[1] -
        0.016748899891587572,
    ) < eps()
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :CVaR)[1] -
        0.02405795664064266,
    ) < eps()
    @test abs(
        calc_risk(
    ...
    @test abs(
        calc_risk(w, portfolio.returns; cov = portfolio.cov, rm = :GMD)[1] -
