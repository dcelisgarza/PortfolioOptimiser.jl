...
@testset "EVaR < RLVaR < WR" begin
    ...
    w2 = optimise!(portfolio, RP(; rm = rm))
    rm = EVaR()
    w3 = optimise!(portfolio, RP(; rm = rm))
    rm = WR()
    w4 = optimise!(portfolio, RP(; rm = rm))
...
@testset "MDD" begin
    ...
    w2t = [0.10305221578285576, 0.06581566981219551, 0.07099099957625851,
           0.048241114652394446, 0.06732985730229697, 0.06932973484933298,
           0.02515034294033264, 0.12492846737683698, 0.032396786632197444,
           0.04506792028955777, 0.13647528104880913, 0.03840233798665657,
           0.03580149044400081, 0.03527348309125291, 0.02525964519810973,
           0.024686526887356668, 0.01652343278283386, 0.02162708905332526,
           0.006761426134115084, 0.00688617815928097]
    @test isapprox(w1.weights, w1t, rtol = 1.0e-4)
    @test isapprox(w2.weights, w2t, rtol = 5.0e-5)
...
@testset "RLDaR" begin
    ...
    w1t = [0.06793607730956304, 0.035313690364559815, 0.0420234557177091,
           0.03066508594734419, 0.04581748657343639, 0.0469481038716857,
           0.02581516516945671, 0.09117722931943303, 0.027341613546162418,
           0.03291528801567443, 0.1719330380575546, 0.035684199269740695,
           0.03230473535971694, 0.04018810228538975, 0.024627771326455387,
           0.047065239418186876, 0.04992593195905292, 0.05846705514626887,
           0.037357676549417046, 0.056493054793192145]
...
@testset "Full Kurt" begin
    ...
    portfolio.risk_budget = []
    w1 = optimise!(portfolio, RP(; rm = rm, str_names = true))
    rc1 = risk_contribution(portfolio; type = :RP, rm = rm)
    lrc1, hrc1 = extrema(rc1)
...
@testset "Skew" begin
    ...
    w1t = [0.04590666801322904, 0.07415835047034322, 0.03239678330399572,
           0.029662259755785923, 0.04774518706312094, 0.03526424453647396,
           0.02131516379007599, 0.08112767280346636, 0.028882964830827505,
           0.030720680788611274, 0.06276022911432362, 0.047500612693288924,
           0.04347967970797795, 0.052066347246952194, 0.07524455834144053,
           0.06814169811466136, 0.048713234865189144, 0.0708948690556333,
           0.03533760555222992, 0.06868118995237288]
...
@testset "GMD" begin
    ...
    w1t = [0.04803561432319247, 0.05110135367478884, 0.04574407979975, 0.04016123661644356,
           0.04820494640651343, 0.04947903979849843, 0.029818720504807616,
           0.06382626547708929, 0.04727559296128686, 0.04657123583463412,
           0.0679271902663865, 0.026939302558393073, 0.02314007043698404,
           0.07304922352445169, 0.02826917567954646, 0.049164682637898444,
           0.05682523100061765, 0.07493235171140689, 0.054016346467150346,
           0.0755183403201604]
...
@testset "TGRG" begin
    ...
    w1t = [0.04253488190779744, 0.0499155646820756, 0.0429837499555012, 0.03478162110512582,
           ...
           0.08031142872919832]
    w2t = [0.004134192571050508, 0.009449502061850235, 0.013178019151488024,
           0.015383693793232334, 0.022529169086036537, 0.030568425317016604,
           0.02647614711153048, 0.054329864953368064, 0.03456858133439547,
           0.04087915768628367, 0.07959632423690799, 0.047210180818849146,
           0.030673337054624376, 0.08496716121241572, 0.05293466907788083,
           0.06603163565149309, 0.07860562636467801, 0.10022566969443325,
           0.07991043434650476, 0.12834820847596104]
    @test isapprox(w1.weights, w1t, rtol = 1.0e-5)
    @test isapprox(w2.weights, w2t, rtol = 5.0e-5)
    @test isapprox(hrc1 / lrc1, 1, rtol = 0.05)
    @test isapprox(hrc2 / lrc2, 20, rtol = 0.1)
...
@testset "OWA" begin
    ...
    w1t = [0.04803528490371915, 0.051103382730954286, 0.045744335917041586,
           0.04015986344427394, 0.048205715377279704, 0.04947943446898425,
           0.029818562472622667, 0.06382452582228117, 0.04727544429311836,
           0.04657146293680372, 0.06792664980568222, 0.026939883482698754,
           0.023140342644429632, 0.07304880395567555, 0.028270033988900947,
           0.04916515462428005, 0.05682452664606954, 0.07493140512300281,
           0.054016755183108944, 0.07551843217907285]
