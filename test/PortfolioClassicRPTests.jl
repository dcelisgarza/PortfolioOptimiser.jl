...
@testset "$(:Classic), $(:RP), $(:MAD)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :MAD)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :MAD)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :MAD)
    ...
    w2t = [0.006401779618828623, 0.012593951743256573, 0.017340890783595773,
           0.019134849456476876, 0.028374307245312213, 0.03189651794987627,
           0.019206796668855844, 0.055391316944302715, 0.03338502811307715,
           0.045151356177118636, 0.0807580519059978, 0.03494263054680027,
           0.02578410334386403, 0.07994586838260086, 0.044758985906496206,
           0.05591483590172484, 0.09228051691281969, 0.12028285344497854,
           0.08191841248798935, 0.11453694646602786]
...
@testset "$(:Classic), $(:RP), $(:SSD)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SSD)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :SSD)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SSD)
    ...
    w2t = [0.005429547829098365, 0.011167805664414806, 0.014459305565898126,
           0.017042626615556288, 0.024075080878288097, 0.031344901036748814,
           0.019078016259587233, 0.059195606446996535, 0.031947654907047304,
           0.04196068100639579, 0.08814196888898153, 0.03899899306400899,
           0.03418229998947243, 0.07927269058844665, 0.05025713223285367,
           0.06338856522866418, 0.08186617870438147, 0.12155493479073504,
           0.07827458257785366, 0.10836142772457111]
...
@testset "$(:Classic), $(:RP), $(:FLPM)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :FLPM)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :FLPM)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :FLPM)
    ...
    w2t = [0.006884077432257186, 0.012954874743778584, 0.018385799362110936,
           0.01831964358547592, 0.0360768453411326, 0.027200592181016465,
           0.02021354469214129, 0.05276903846646005, 0.03376615707481203,
           0.0442388899755955, 0.08234210706887593, 0.031754949162667905,
           0.024158856974061862, 0.07390417921315301, 0.03807087872319826,
           0.05745974916872209, 0.09954209209763215, 0.12219715695811513,
           0.0848210415751922, 0.11493952620360075]
    @test isapprox(w1.weights, w1t, rtol = 0.0001)
    @test isapprox(w2.weights, w2t, rtol = 1.0e-6)
    @test isapprox(hrc1 / lrc1, 1, atol = 1e-2)
    @test isapprox(hrc2 / lrc2, 20, atol = 1e-2)
...
@testset "$(:Classic), $(:RP), $(:SLPM)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SLPM)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :SLPM)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SLPM)
    ...
    w2t = [0.005492527475806188, 0.011429816988193184, 0.014822086266724446,
           0.017306287129466233, 0.026322726877075143, 0.02905990211343986,
           0.019770052943997973, 0.0592124634100771, 0.032193671946941074,
           0.04165106177764175, 0.08829033644315885, 0.037309763647838966,
           0.03234339539756486, 0.07666976289341881, 0.04703097020934251,
           0.06578728165840875, 0.08445214229212933, 0.12165795866775381,
           0.07958227554108589, 0.10961551631993534]
...
@testset "$(:Classic), $(:RP), $(:WR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :WR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :WR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :WR)
...
@testset "$(:Classic), $(:RP), $(:CVaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :CVaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :CVaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :CVaR)
...
@testset "$(:Classic), $(:RP), $(:EVaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :EVaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :EVaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :EVaR)
...
@testset "$(:Classic), $(:RP), $(:RVaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :RVaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :RVaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :RVaR)
...
@testset "$(:Classic), $(:RP), $(:ADD)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :ADD)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :ADD)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :ADD)
...
@testset "$(:Classic), $(:RP), $(:CDaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :CDaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :CDaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :CDaR)
...
@testset "$(:Classic), $(:RP), $(:UCI)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :UCI)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :UCI)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :UCI)
...
@testset "$(:Classic), $(:RP), $(:EDaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :EDaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :EDaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :EDaR)
...
@testset "$(:Classic), $(:RP), $(:RDaR)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :RDaR)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :RDaR)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :RDaR)
...
@testset "$(:Classic), $(:RP), Full $(:Kurt)" begin
    ...
    w1t = [0.03879158773899491, 0.04946318916187915, 0.03767536457743636,
           0.04975768359685481, 0.03583384747996175, 0.05474667190193154,
           0.02469826359420486, 0.10506491736193022, 0.031245766025529604,
           0.04312788495096333, 0.12822307815405873, 0.03170133005454372,
           0.026067725442004967, 0.057123092045424234, 0.03137705105386256,
           0.04155724092469867, 0.044681796838160794, 0.0754338209703899,
           0.03624092724713855, 0.057188760880031476]

    w2t = [0.004127710286387879, 0.010592152386952021, 0.012536905345418492,
           0.023303462236461917, 0.01936823663730284, 0.03214466953862615,
           0.018650835191729918, 0.08347430641751365, 0.026201862079995652,
           0.04168068597107915, 0.1352680942007192, 0.03614055044122551,
           0.030447496750462644, 0.07180951106902754, 0.03968594759203002,
           0.05644735602737195, 0.07166639041345427, 0.11896200641502389,
           0.06340744330857792, 0.10408437769063927]

    @test isapprox(w1.weights, w1t, rtol = 1.0e-5)
    @test isapprox(w2.weights, w2t, rtol = 1.0e-5)
    @test isapprox(hrc1 / lrc1, 1, atol = 1.6e0)
...
@testset "$(:Classic), $(:RP), Reduced $(:Kurt)" begin
    ...
    w1t = [0.0500000000000011, 0.05000000000000058, 0.0500000000000005,
           0.050000000000001134, 0.05000000000000087, 0.0500000000000012,
           0.05000000000000044, 0.050000000000000794, 0.05000000000000046,
           0.05000000000000119, 0.0500000000000009, 0.050000000000001155,
           0.049999999999997845, 0.04999999999999776, 0.050000000000000905,
           0.04999999999999878, 0.049999999999997484, 0.049999999999998525,
           0.05000000000000055, 0.04999999999999802]

    w2t = [0.02610708275409532, 0.02837943088274139, 0.030651303698748616,
           0.03292296592900042, 0.03519875334620729, 0.03748347710210248,
           0.039781527753448405, 0.042099870335476915, 0.044453433199575766,
           0.04686708624986517, 0.04937400472746767, 0.05201270392712209,
           0.054818393390558565, 0.05781799869836054, 0.061024063467002496,
           0.06442370351110903, 0.06809573346857908, 0.0719652815702216,
           0.07608875958076423, 0.08043442640755293]
...
@testset "$(:Classic), $(:RP), Full $(:SKurt)" begin
    ...
    w1 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SKurt)
    rc1 = risk_contribution(portfolio; type = :RP, rm = :SKurt)
    lrc1, hrc1 = extrema(rc1)

    portfolio.risk_budget = 1:size(portfolio.returns, 2)
    w2 = optimise!(portfolio; class = :Classic, type = :RP, rm = :SKurt)
    rc2 = risk_contribution(portfolio; type = :RP, rm = :SKurt)
    lrc2, hrc2 = extrema(rc2)

    w1t = [0.041170947272951605, 0.055151102657145686, 0.03582026711008352,
           0.03810281647100041, 0.03551958231813474, 0.054217762490842675,
           0.021569898487818123, 0.09467152302998202, 0.030283020031477308,
           0.03821071007217146, 0.1240516019281326, 0.034933363590817265,
           0.028499637129673648, 0.06128622298896649, 0.03461265007418104,
           0.044610124033738624, 0.04779839493700672, 0.07608146521350194,
           0.03653809970499657, 0.06687081045737765]

    w2t = [0.004175205263429701, 0.011765748187100822, 0.011015252491488306,
           0.01577359455014908, 0.017593349506285338, 0.03187177049780766,
           0.01523068356780147, 0.0720093728891317, 0.02536718681887658,
           0.035781514103165026, 0.13000234588810786, 0.037090729040813834,
           0.0335443668308728, 0.0755080628731996, 0.043604346172737944,
           0.06299741499421682, 0.07703658809323406, 0.11862869961574692,
           0.06432382504657572, 0.11667994356925879]
