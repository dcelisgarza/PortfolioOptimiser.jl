...
function optimise_allocation(port, key, tickers, latest_prices)
    ...
    for solver ∈ solvers
        ...
        try
            ...
            push!(solvers_tried,
                  solution_key => Dict(:objective_val => objective_value(model),
...
function lp_sub_allocation!(port, key, tickers, weights, latest_prices, investment,
    ...
    shares, cost, allocated_weights, available_funds = optimise_allocation(port, key,
                                                                           tickers,
                                                                           latest_prices)
    if !isempty(shares)
...
function lp_allocation!(port, port_key, investment, string_names)
    ...
    if !isempty(port.alloc_optimal[key])
        port.alloc_leftover[key] = long_leftover + short_leftover
        if !iszero(fees)
            port.alloc_optimal["$(key)_fees"] = fees
        end
    end

    return port.alloc_optimal[key]
end
function allocate!(port::AbstractPortfolio, ::LP;
                   key::Union{Symbol, <:AbstractString} = :Trad, investment::Real = 1e6,
