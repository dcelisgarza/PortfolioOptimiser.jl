...
function get_cluster_portfolio(port, internal_args, i, cluster, cidx, idx_sq, Nc,
    ...
    (; assets, returns, f_assets, f_returns, loadings, regression_type, mu_l, mu, cov, cor, dist, k, max_num_assets_kurt, max_num_assets_kurt_scale, kurt, skurt, L_2, S_2, skew, V, sskew, SV, f_mu, f_cov, fm_returns, fm_mu, fm_cov, bl_bench_weights, bl_mu, bl_cov, blfm_mu, blfm_cov, cov_l, cov_u, cov_mu, cov_sigma, d_mu, k_mu, k_sigma, w_min, w_max, risk_budget, f_risk_budget, short, long_lb, long_ub, short_ub, short_lb, budget_lb, budget, budget_ub, short_budget_ub, short_budget, short_budget_lb, card_scale, card, nea, tracking, turnover, l1, l2, long_fees, short_fees, rebalance, solvers) = port
    ...
    if cvx_flag
        long_lb = get_cluster_real_or_vector(long_lb, cidx)
        long_ub = get_cluster_real_or_vector(long_ub, cidx)
        short_ub = get_cluster_real_or_vector(short_ub, cidx)
    ...
    intra_port = Portfolio(; assets = assets, ret = returns, f_assets = f_assets,
                           ...
                           short = short, long_lb = long_lb, long_ub = long_ub,
                           short_ub = short_ub, short_lb = short_lb, budget_lb = budget_lb,
...
function get_external_portfolio(port, wi, external_args, special_rm_idx)
    ...
    (; assets, returns, f_assets, f_returns, loadings, regression_type, mu_l, mu, cov, k, max_num_assets_kurt, max_num_assets_kurt_scale, f_mu, f_cov, fm_returns, fm_mu, fm_cov, bl_bench_weights, bl_mu, bl_cov, blfm_mu, blfm_cov, w_min, w_max, risk_budget, f_risk_budget, short, long_lb, long_ub, short_ub, short_lb, budget_lb, budget, budget_ub, short_budget_ub, short_budget, short_budget_lb, card_scale, card, nea, tracking, turnover, l1, l2, long_fees, short_fees, rebalance, solvers) = port
    ...
    if cvx_flag
        long_lb = get_external_real_or_vector(long_lb, wi)
        long_ub = get_external_real_or_vector(long_ub, wi)
        short_ub = get_external_real_or_vector(short_ub, wi)
    ...
    inter_port = Portfolio(; assets = assets, ret = returns, f_assets = f_assets,
                           ...
                           f_risk_budget = f_risk_budget, short = short, long_lb = long_lb,
                           long_ub = long_ub, short_ub = short_ub, short_lb = short_lb,
