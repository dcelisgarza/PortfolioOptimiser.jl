...
mutable struct Portfolio{
                         ...
                         T_f_timestamps, T_f_returns, T_asset_sets, T_a_ineq_constraints,
                         ...
                         # Budget and shorting
                         T_short, T_long_t, T_long_ub, T_short_t, T_short_lb, T_budget_lb,
    ...
    a_ineq_constraints::T_a_ineq_constraints
    a_eq_constraints::T_a_eq_constraints
    a_card_ineq_constraints::T_a_card_ineq_constraints
    a_card_eq_constraints::T_a_card_eq_constraints
...
function long_short_budget_assert(N, long_t, long_ub, budget_lb, budget, budget_ub, short,
                                  short_t, short_lb, short_budget_ub, short_budget,
                                  short_budget_lb)
    real_or_vector_assert(long_t, N, :long_t, >=, zero(eltype(long_t)))
    real_or_vector_assert(long_ub, N, :long_ub, >=, zero(eltype(long_ub)))
    @smart_assert(all(long_t .<= long_ub))
    ...
    if short
        real_or_vector_assert(short_t, N, :short_t, <=, zero(eltype(short_t)))
        real_or_vector_assert(short_lb, N, :short_lb, <=, zero(eltype(short_lb)))
        @smart_assert(short_lb <= short_t)
...
function Portfolio(;
                   ...
                   a_ineq_constraints::DataFrame = DataFrame(),
                   a_eq_constraints::DataFrame = DataFrame(),
                   a_card_ineq_constraints::DataFrame = DataFrame(),
                   a_card_eq_constraints::DataFrame = DataFrame(),
    ...
    # Budget and shorting
    budget, short_budget = long_short_budget_assert(N, long_t, long_ub, budget_lb, budget,
                                                    budget_ub, short, short_t, short_lb,
                                                    short_budget_ub, short_budget,
                                                    short_budget_lb)
    # Cardinality
    @smart_assert(N >= card >= zero(card))
    ...
    return Portfolio{
                     ...
                     typeof(f_returns),
                     # Constraint generation
                     typeof(asset_sets), typeof(a_ineq_constraints),
                     ...
                     typeof(alloc_optimal), typeof(alloc_leftover), typeof(alloc_fail)}(
                                                                                        ...
                                                                                        a_ineq_constraints,
                                                                                        a_eq_constraints,
                                                                                        a_card_ineq_constraints,
                                                                                        a_card_eq_constraints,
                                                                                        f_ineq_constraints,
                                                                                        f_eq_constraints,
...
function Base.setproperty!(port::Portfolio, sym::Symbol, val)
    ...
    elseif sym == :budget
        ...
        val = long_short_budget_assert(N, port.long_t, port.long_ub, port.budget_lb, val,
                                       port.budget_ub, port.short, port.short_t,
    ...
    elseif sym == :short_budget
        ...
        val = long_short_budget_assert(N, port.long_t, port.long_ub, port.budget_lb,
                                       ...
                                       port.short_t, port.short_lb, port.short_budget_ub,
...
function Base.deepcopy(port::Portfolio)
    return Portfolio{
                     ...
                     # Constraint generation
                     typeof(port.asset_sets), typeof(port.a_ineq_constraints),
                     ...
                     typeof(port.alloc_fail)}(
                                              ...
                                              deepcopy(port.a_ineq_constraints),
                                              deepcopy(port.a_eq_constraints),
                                              deepcopy(port.a_card_ineq_constraints),
                                              deepcopy(port.a_card_eq_constraints),
                                              ...
                                              # Budget and shorting
                                              deepcopy(port.short), deepcopy(port.long_t),
                                              deepcopy(port.long_ub),
                                              deepcopy(port.short_t),
