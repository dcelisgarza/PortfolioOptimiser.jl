...
function _mv_setup(portfolio, sigma, rm, kelly, obj)
    ...
    G = sqrt(sigma)
    @constraint(model, [tdev; G * model[:w]] in SecondOrderCone())

    if isfinite(dev_u)
        if obj == :sharpe
            @constraint(model, tdev <= dev_u * model[:k])
        else
...
function _mad_setup(portfolio, rm, T, returns, mu, obj)
    ...
    @constraint(model, abs_dev * model[:w] >= -tmad)
    ...
    if isfinite(sdev_u)
        if obj == :sharpe
            @constraint(model, msd_risk <= sdev_u * model[:k])
        else
...
function _wr_setup(portfolio, rm, returns, obj)
    ...
    hist_ret = model[:hist_ret]
    @constraint(model, -hist_ret .<= twr)
    @expression(model, wr_risk, twr)

    if isfinite(wr_u)
        if obj == :sharpe
            @constraint(model, -hist_ret .<= wr_u * model[:k])
        else
            @constraint(model, -hist_ret .<= wr_u)
        end
    end
...
function _lpm_setup(portfolio, rm, T, returns, obj, rf)
    ...
    hist_ret = model[:hist_ret]
    if obj == :sharpe
        @constraint(model, tlpm .>= rf * model[:k] .- hist_ret)
    else
        @constraint(model, tlpm .>= rf .- hist_ret)
    end
...
function _drawdown_setup(portfolio, rm, T, returns, obj)
    ...
    if rm == :cdar || isfinite(cdar_u)
        ...
        @expression(model, cdar_risk, tdar + sum(zdar) * invat)
...
function _owa_setup(portfolio, rm, T, returns, obj)
    ...
    if rm == :gmd || isfinite(gmd_u)
        ...
        if isfinite(gmd_u)
            if obj == :sharpe
                @constraint(model, gmd_risk <= gmd_u * model[:k] / 2)
            else
