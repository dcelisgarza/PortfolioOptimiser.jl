    ...
    if isone(count)
        ...
        if isone(idx)
            ...
            if !iszero(nmax) && N > nmax
                ...
                @variable(model, zkurt[1:size(port.L_2, 1), 1:count])
            end
        end
        if !iszero(nmax) && N > nmax
            @constraint(model,
            ...
            S_2 = port.S_2
            sqrt_sigma_4 = sqrt(S_2 * kt * transpose(S_2))
            @constraint(model, view(model[:zkurt], :, idx) .== L_2 * vec(model[:W]))
            @constraint(model,
                        [model[:kurt_risk][idx];
    ...
    if isone(count)
        ...
        if isone(idx)
            ...
            if !iszero(nmax) && N > nmax
                ...
                @variable(model, zskurt[1:size(port.L_2, 1), 1:count])
            end
        end
        if !iszero(nmax) && N > nmax
            @constraint(model,
            ...
            S_2 = port.S_2
            sqrt_sigma_4 = sqrt(S_2 * kt * transpose(S_2))
            @constraint(model, view(model[:zskurt], :, idx) .== L_2 * vec(model[:W]))
            @constraint(model,
...
function set_rm(port::Portfolio2, rm::OWA2, type::Union{Trad2, RP2}, obj::ObjectiveFunction,
    ...
    if isone(count)
        ...
        if isone(idx)
            @variable(model, owa_risk[1:count])
            if !rm.owa.approx
                _owa_setup(model, T)
