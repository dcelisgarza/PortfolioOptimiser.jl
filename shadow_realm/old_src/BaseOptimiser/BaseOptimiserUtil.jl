...
function _create_weight_bounds(num_tickers, bounds)
    ...
    elseif length(bounds) == 2
        ...
        throw(ArgumentError("Weight bounds must either be a tuple of length 2, or a vector of length $num_tickers of tuples/vectors of length 2."))
    end
...
function _make_weight_sum_constraint!(model, market_neutral)
    ...
    if !market_neutral
        ...
        port_possible = any(getfield.(getfield.(constraint_object.(lower_boundsConst),
                                                :set), :lower) .< 0)
...
```julia
extra_constraints = [:(model[:w][2] >= 2), :(sum(model[:w]) <= 5)]
ef = EffMeanVar(tickers, mean_ret, cov_mtx; extra_constraints = extra_constraints)
```
...
function _val_compare_benchmark(val, op, benchmark, correction, name)
    if op(val, benchmark)
        @warn("Value of $name, $val $(String(Symbol(op))) $benchmark. Correcting to $correction.")
        val = correction
    end
    return val
end
...
function _setup_and_optimise(model, optimiser, silent, optimiser_attributes = ())
    ...
    if term_status ∉ (MOI.OPTIMAL, MOI.LOCALLY_SOLVED)
        @warn("The optimiser could not solve the problem satisfactorily: $term_status. The solution may be useful regardless.")
    end
end
