...
function ret_model(::EMRet, returns; compound = true, frequency = 1,
                   span = ceil(Int, 4 * size(returns, 1) / log(size(returns, 1) + 2)),)
    N = size(returns, 1)
    if compound
...
function ret_model(::CAPMRet, returns, market_returns = nothing; rf = 1.02^(1 / 252) - 1,
                   compound = true, frequency = 1, cov_method::AbstractRiskModel = Cov(),
                   target = 1.02^(1 / 252) - 1, fix_method::AbstractFixPosDef = SFix(),
                   span = ceil(Int, 4 * size(returns, 1) / log(size(returns, 1) + 2)),
                   scale = nothing, custom_cov_estimator = nothing, custom_cov_args = (),
                   custom_cov_kwargs = (),)
    β, returns = _compute_betas(market_returns, returns, cov_method, target, fix_method,
                                span, scale, custom_cov_estimator, custom_cov_args,
                                custom_cov_kwargs)
...
function ret_model(::ECAPMRet, returns, market_returns = nothing; rf = 1.02^(1 / 252) - 1,
                   compound = true, frequency = 1,
                   rspan = ceil(Int, 4 * size(returns, 1) / log(size(returns, 1) + 2)),
                   cov_method::AbstractRiskModel = ECov(), target = 1.02^(1 / 252) - 1,
                   fix_method::AbstractFixPosDef = SFix(),
                   cspan = ceil(Int, 4 * size(returns, 1) / log(size(returns, 1) + 2)),
                   scale = nothing, custom_cov_estimator = nothing, custom_cov_args = (),
                   custom_cov_kwargs = (),)
    β, returns = _compute_betas(market_returns, returns, cov_method, target, fix_method,
                                cspan, scale, custom_cov_estimator, custom_cov_args,
                                custom_cov_kwargs)

    # Exponentially weighted mean market return.
    mkt_mean_ret = ret_model(EMRet(), returns[:, end]; compound = compound,
                             frequency = frequency, span = rspan,)[1]

    # Capital asset pricing.
    return rf .+ β * (mkt_mean_ret - rf)
end

function _compute_betas(market_returns, returns, cov_method, target, fix_method, cspan,
                        scale, custom_cov_estimator, custom_cov_args, custom_cov_kwargs)
    ...
    cov_mtx = cov(cov_method, returns, target, fix_method, cspan, scale,
                  custom_cov_estimator, custom_cov_args...; custom_cov_kwargs...,)
...
"""
function returns_from_prices(prices, log_ret = false; capm = false,
                             market_returns = nothing, rf = 1.02^(1 / 252) - 1,
                             cov_method::AbstractRiskModel = ECov(),
                             target = 1.02^(1 / 252) - 1,
                             fix_method::AbstractFixPosDef = SFix(),
