...
function CriticalLine(tickers, mean_ret, cov_mtx; weight_bounds = (0, 1))
    ...
    return CriticalLine(tickers, mean_ret, zeros(num_tickers), cov_mtx, lower_bounds,
                        upper_bounds, Vector{Vector{Float64}}(),
                        Vector{Union{Float64, Nothing}}(),
                        Vector{Union{Float64, Nothing}}(), Vector{Vector{Float64}}(),
                        Vector{NamedTuple{(:mu, :sigma, :weights),
                                          Tuple{Vector{Float64}, Vector{Float64},
                                                Vector{Vector{Float64}}}}}(undef, 1))
end
